...
<div id="wrapper">
    ...
    <h2 style="margin-left: 37%">TUGAS AKHIR</h2>
            ...
            <li class="active"><a href="reportTAHarian.php">Laporan Harian</a></li>
    ...
    <div class="table-responsive">
        <div class="col-md-6">
                <a href="cetakTAH.php"><button type="button" name="tambah" class="btn-primary btn" style="margin-left: -5%"> Cetak</button></a>
            </div>
            <br><br>
           <table class="table">
              <thead>
        ...
        $result  = mysqli_query($con,"SELECT * FROM log JOIN user ON log.id_user=user.id JOIN koleksita ON log.id_koleksi=koleksita.id_koleksiTA");
        $total   = mysqli_num_rows($result);
        $pages   = ceil($total/$halaman);
        $query   = mysqli_query($con,"SELECT * FROM log JOIN user ON log.id_user=user.id JOIN koleksita ON koleksita.id_koleksiTA=log.id_koleksi ORDER BY waktu DESC LIMIT $mulai, $halaman");
        $no      = $mulai+0;

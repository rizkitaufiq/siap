         ...
         <div class="row">
            ...
            <div class="col-md-6">
                <div class="search pull-right">
                    <form action="koleksiTA.php" method="GET">
        ...
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1; 
                $adjacents = isset($_GET['adjacents']) ? intval($_GET['adjacents']) : 3; 
                    ...
                    if(isset($_REQUEST['cari']) && $_REQUEST['cari']<>""){
                        ...
                        $lihat = mysqli_query($con, "SELECT * FROM koleksita WHERE judul LIKE '%$cari%' OR penulis LIKE '%$cari%' OR jurusan LIKE '%$cari%' OR dosbimTA LIKE '%$cari%' ORDER BY judul ASC"); 
                        $reload = $_SERVER['PHP_SELF'] . "?cari=".$cari."&amp;adjacents=".$adjacents; 
                    } else{
                        $lihat = mysqli_query($con, "SELECT * FROM koleksita ORDER BY judul ASC"); 
            ...
            echo "<tr>";
                ...
                echo "<td><button name='lihat' data-id='".$hasil['id_koleksiTA']."' data-toggle='modal' data-target='#lihat' class='btn-succes btn fa fa-eye' ></button></td>";
                echo "<td><button name='edit' data-id='".$hasil['id_koleksiTA']."' data-toggle='modal' data-target='#edit' class='btn-primary btn fa fa-pencil'></button></td>";
                echo "<td><button name='hapus' data-id='".$hasil['id_koleksiTA']."' data-toggle='modal' data-target='#hapus' class='btn-danger btn fa fa-trash-o'></button></td>";
            echo "</tr>";
            $i++;
            $count++;
            }
    ...
    <div class="modal fade" id="tambahmodal">
        <div class="modal-dialog">
            ...
            <div class="modal-body">
                <form method="post" id="insert_form" action="tambahTA.php" enctype="multipart/form-data">
                    ...
                    <input type="text" name="dosbimTA" class="form-control1" required="">
                    <br><br>
                     <label>Dosen Pembimbing II</label>
                    <input type="text" name="dosbimTA1" class="form-control1" required="">
                    <br><br>
    ...
    <div class="modal fade" id="edit">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h3 style="text-align: center;" class="modal-title">Edit Arsip TA</h4>
                </div>
            <div class="modal-body">
                <div class="fetched-data"></div>
            </div>

...
if($_POST['rowid']){
 	...
 	$sql = "SELECT * FROM tmp_koleksiebook WHERE id_tmpkoleksiebook = $id";
 	$result = $con->query($sql);
 	foreach ($result as $lihat){ ?>


    <form method="post" action="hapusEBOOKpus1.php">
        <?php echo "<input type='hidden' name='id' value='".$lihat['id_tmpkoleksiebook']."'>"?>
        <table class="table">
 		<tbody>
 			<tr>
 				<td>Judul</td>

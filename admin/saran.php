...
<div id="wrapper">
     <!-- Navigation -->
        ...
        <div class="graphs">
        <div class="col_3">
            <h1 style="margin-left: 43%">SARAN</h1>
        ...
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1; 
                $adjacents = isset($_GET['adjacents']) ? intval($_GET['adjacents']) : 3; 
                    ...
                    if(isset($_REQUEST['cari']) && $_REQUEST['cari']<>""){
                        ...
                        $lihat = mysqli_query($con, "SELECT * FROM saran JOIN user ON saran.id_user=user.id WHERE username LIKE '%$cari%' OR saran LIKE '%$cari%' OR waktu LIKE '%$cari%' ORDER BY username ASC"); 
                        $reload = $_SERVER['PHP_SELF'] . "?cari=".$cari."&amp;adjacents=".$adjacents; 
                    } else{
                        $lihat = mysqli_query($con, "SELECT * FROM saran JOIN user ON saran.id_user=user.id ORDER BY waktu DESC"); 
                ...
                echo "<td><button name='hapus' data-id='".$hasil['id_saran']."' data-toggle='modal' data-target='#hapus' class='btn-danger btn fa fa-trash-o'></button></td>";
            echo "</tr>";
            $i++;
            $count++;
            }
